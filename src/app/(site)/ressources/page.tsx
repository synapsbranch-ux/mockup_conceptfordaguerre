      ...
      <main id="contenu" className="section-pad">
        <div className="shell">
          <header style={{ maxWidth: '46rem' }}>
            ...
            <p className="lede">
              Guides, modèles et rapports publiés au fil des travaux. Aucun compte n’est nécessaire
              pour les consulter.
            </p>
          </header>
                ...
                >
                  ...
                  >
                    <div style={{ minWidth: 0 }}>
                      <h2 style={{ fontSize: '1.05rem', margin: 0 }}>{document.title}</h2>
                      ...
                      >
                        {CATEGORY_LABELS[document.category as string] ?? 'Document'}
                        {humanSize(document.filesize) ? ` · ${humanSize(document.filesize)}` : ''}
                        {document.visibility === 'authenticated' ? ' · réservé aux comptes' : ''}
                      </p>
                    </div>
          ...
          {!sessionUser && (
            <p className="muted" style={{ marginTop: '2.5rem' }}>
              <Link href="/connexion?next=%2Fressources">Connectez-vous</Link> pour accéder aux
              ressources réservées aux comptes.
            </p>
          )}
        </div>
      </main>
