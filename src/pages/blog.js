  ...
  return (
    <Layout>
      ...
      <section className="blog-feature shell">
        <Link href={`/blog/${featured.slug}`} className="blog-feature-image"><SiteImage src={featured.image} sizes="(max-width: 820px) 100vw, 62vw" priority /></Link>
      ...
      <section className="blog-list shell section-pad">
        ...
        {rest.map((article, index) => (
          <Link className="blog-row" href={`/blog/${article.slug}`} key={article.slug}>
            <span className="blog-row-number">0{index + 2}</span><SiteImage src={article.image} sizes="250px" /><div><span>{article.category}</span><h2>{article.title}</h2><p>{article.excerpt}</p></div><div className="blog-row-meta"><span>{article.read}</span><Arrow /></div>
          </Link>
        ))}
      </section>
    </Layout>
