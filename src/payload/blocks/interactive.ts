 ...
 * Le design public est conservé à l'identique ; seuls les textes, libellés et
 * mentions sont administrables. Aucune authentification client n'est branchée.
 */
export const AuthPrototypeBlock: Block = {
  slug: 'authPrototype',
  interfaceName: 'AuthPrototypeBlock',
  labels: { singular: 'Espace utilisateur (maquette)', plural: 'Espaces utilisateur (maquettes)' },
  fields: [
    ...blockBaseFields,
    { name: 'eyebrow', type: 'text', label: 'Surtitre', maxLength: 60 },
    headlineField({ label: 'Titre' }),
    ...
    },
    {
      ...
      label: 'Onglets',
      fields: [
        { name: 'login', type: 'text', label: 'Onglet connexion', required: true, maxLength: 40 },
        { name: 'register', type: 'text', label: 'Onglet inscription', required: true, maxLength: 40 },
      ],
    },
    {
      name: 'fields',
      ...
      fields: [
        ...
        },
        {
          type: 'row',
          fields: [
            { name: 'email', type: 'text', label: 'Courriel', admin: { width: '50%' } },
            { name: 'emailPlaceholder', type: 'text', label: 'Indication — courriel', admin: { width: '50%' } },
          ],
        },
        {
          type: 'row',
          fields: [
            { name: 'password', type: 'text', label: 'Mot de passe', admin: { width: '50%' } },
            { name: 'passwordPlaceholder', type: 'text', label: 'Indication — mot de passe', admin: { width: '50%' } },
          ],
        },
        { name: 'consent', type: 'textarea', label: 'Texte de consentement', maxLength: 400 },
      ],
    },
    {
      name: 'buttons',
      ...
      label: 'Boutons',
      fields: [
        { name: 'login', type: 'text', label: 'Bouton connexion', required: true, maxLength: 60 },
        { name: 'register', type: 'text', label: 'Bouton inscription', required: true, maxLength: 60 },
      ],
    },
    {
      name: 'caption',
      type: 'text',
      label: 'Mention de bas de carte',
      maxLength: 200,
      admin: { description: 'Signale que l’authentification n’est pas encore branchée.' },
    },
  ],
}
